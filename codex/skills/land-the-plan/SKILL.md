...
### Step 3 — Update task manifest metadata
...
- commit `goals/task-manifest.csv`
- push manifest update to remote when a new manifest commit is created

If manifest update/commit/push fails, emit `BLOCKED`.
...
### Step 7 — Create or update the PR via GitHub MCP prompt

Do not run `gh pr` commands in this stage.
Prompt GitHub MCP to create or update the PR against resolved base branch using generated title/body.
...
- Gate 4: `task-manifest-land-update.sh` completed (manifest update + commit + required push behavior).
...
- Gate 9: PR created/updated successfully via GitHub MCP.
...
- Do not run `gh pr` commands for PR creation/update; use GitHub MCP prompt flow.
