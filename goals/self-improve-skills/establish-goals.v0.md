...
- Constraints (sandbox, commands, policy): stage order is mandatory; goal lock must occur before planning or implementation; establish-goals artifacts must be script-backed; the skills are centralized but improvement evidence is recorded in `ericpassmore/prompts`
...
1. Please confirm these goals so I can lock them and hand off to `prepare-takeoff`.
...
- Await user confirmation, then set `State = locked`, re-run extraction and validation, and hand off to `prepare-takeoff`.
