...
- Iteration: v0
- State: locked
- Task name (proposed, kebab-case): improve-feb-15-2026

## Request restatement

- Perform a comprehensive review of `./codex` and `codex/AGENTS.md` against `codex/principles.md`.
- Prioritize: (1) consistency with principles, (2) execution speed improvements, (3) consolidation/trimming of redundant instructions.
- Preserve existing behavior and all lifecycle stages.
- Account for repository-specific codex root usage where this repo uses `./codex`, while many implementations prefer `./.codex` with fallback to `$HOME/.codex`.

## Context considered

- Repo/rules/skills consulted: `codex/AGENTS.md`, `codex/principles.md`, `codex/skills/*/SKILL.md`, `codex/goals/*.md`
- Relevant files (if any): `codex/scripts/*` (expected implementation surface), `codex/codex-config.yaml`, `codex/project-structure.md`
- Constraints (sandbox, commands, policy): ACAC lifecycle order is mandatory; script-backed stage operations are mandatory; no stage skipping; no behavior/stage contract regressions allowed.
...
1. "Performance speedup" applies to agent workflow efficiency (instruction clarity, reduced duplication, reduced unnecessary command churn), not runtime optimization of unrelated product code.
2. "Comprehensive review `./codex`" targets governance artifacts first (`AGENTS.md`, `principles.md`, stage skills, and supporting scripts) and allows scoped edits where evidence supports consistency/speed/consolidation gains.
...
1. Existing stage names, verdict tokens, and lifecycle order are immutable.
2. Any optimization must preserve observable stage behavior and validation semantics.
3. Root-resolution compatibility (`./.codex`, `./codex`, `$HOME/.codex`) remains supported; this repo-specific guidance may be clarified for `./codex`-first usage without breaking compatibility.
...
- Over-consolidation could unintentionally weaken normative requirements; mitigation is validator-backed checks and exact-contract preservation.
- Performance-oriented trimming could hide nuance; mitigation is keeping hard requirements centralized and explicit.
