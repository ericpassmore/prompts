...
- Update the Codex lifecycle skills so git operations are routed through approved git wrapper/helper scripts instead of direct raw git command instructions, and correct the `git-commit` development environment exception spelling from `developement.env` to `development.env`.
...
- Await user approval to lock goals.
